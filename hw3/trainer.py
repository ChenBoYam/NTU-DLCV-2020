...
class GTA(object):
    ...
    def __init__(self, opt, nclasses, mean, std, source_trainloader, source_valloader, targetloader,target_testloader):

        self.source_trainloader = source_trainloader
        self.source_valloader = source_valloader
        self.targetloader = targetloader
        self.target_testloader = target_testloader
        self.opt = opt
        self.mean = mean
        self.std = std
        self.best_val = 0
    ...
    def validate(self, epoch,k):
        ...
        print('%s| Epoch: %d_%d, Val Accuracy: %f %%' % (datetime.datetime.now(), epoch,k, val_acc))
    
        # Saving checkpoints
        torch.save(self.netF.state_dict(), '%s/models_s2u/netF_%d_%d.pth' %(self.opt.outf,epoch,k))
        torch.save(self.netC.state_dict(), '%s/models_s2u/netC_%d_%d.pth' %(self.opt.outf,epoch,k))
        
        if val_acc>self.best_val:
            self.best_val = val_acc
            torch.save(self.netF.state_dict(), '%s/models_s2u/model_best_netF.pth' %(self.opt.outf))
            torch.save(self.netC.state_dict(), '%s/models_s2u/model_best_netC.pth' %(self.opt.outf))
    ...
    def train(self):
        ...
        for epoch in range(self.opt.nepochs):
            ...
            for i, (datas, datat) in enumerate(zip(self.source_trainloader, self.targetloader)):
                ...
                # errF_src_fromD = self.criterion_c(src_fakeoutputD_c, src_labelsv)
                ...
                # Visualization
                if i == 1:
                    vutils.save_image((src_gen.data/2)+0.5, '%s/visualization_s2u/source_gen_%d.png' %(self.opt.outf, epoch))
                    vutils.save_image((tgt_gen.data/2)+0.5, '%s/visualization_s2u/target_gen_%d.png' %(self.opt.outf, epoch))
                ...
                if(i % 80 == 0):
                    ...
                    evaluate(self.target_testloader,self.opt,epoch,i)
...
def evaluate(target_testloader,opt,epoch,i) : 
    targetloader = target_testloader
    ...
    if opt.method == 'GTA':
        netF_path = os.path.join(opt.checkpoint_dir, 'netF_%d_%d.pth'%(epoch,i))
        netC_path = os.path.join(opt.checkpoint_dir, 'netC_%d_%d.pth'%(epoch,i))
        # if opt.model_best == 0: 
    ...
    for k, datas in enumerate(targetloader):
        inputs, labels = datas
        if opt.gpu>=0:
    ...
    print('Test Accuracy: %f %%' % (test_acc),'\t %d_%d'%(epoch,i))
    acc.append(test_acc)
    plt.figure(figsize=(10,5))
    plt.title("acurate")
    plt.plot(acc)
    plt.savefig('./GTA_s2u_acc.jpg')