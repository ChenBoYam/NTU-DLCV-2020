...
def train_epoch(train_source_dataloader, train_target_dataloader, test_target_dataloader, lamb):
    '''
      Args:
        train_source_dataloader: source data的dataloader
        train_target_dataloader: target data的dataloader
...
for epoch in range(25):
    ...
    torch.save(feature_extractor.state_dict(), f'FtExtr_{source}_to_{target}_lb.pkl')
    torch.save(label_predictor.state_dict(), f'LP_{source}_to_{target}_lb.pkl')
...
result = []
feature_extractor.load_state_dict(torch.load(f'FtExtr_{source}_to_{target}_lb.pkl'))
label_predictor.load_state_dict(torch.load(f'LP_{source}_to_{target}_lb.pkl'))
label_predictor.eval()
feature_extractor.eval()
test_total_hit, test_total_num = 0.0, 0.0
for i, (test_data, test_label) in enumerate(test_target_dataloader):
