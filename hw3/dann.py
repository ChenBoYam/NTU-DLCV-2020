...
root = sys.argv[1] #"./hw3_data/digits/mnistm/test"
target = sys.argv[2] #"mnistm"
...
onlyfiles = [f for f in listdir(target_path) if isfile(join(root, f))]
...
'''
result = []
feature_extractor.load_state_dict(torch.load(f'FtExtr_{source}_to_{target}.pkl?dl=1'))
label_predictor.load_state_dict(torch.load(f'LP_{source}_to_{target}.pkl?dl=1'))
label_predictor.eval()
feature_extractor.eval()
test_total_hit, test_total_num = 0.0, 0.0
for i, test_data in enumerate(test_target_dataloader):
    test_data = test_data.cuda()
    class_logits = label_predictor(feature_extractor(test_data))
